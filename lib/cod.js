...
(function() {
  ...
  L.$.rename = function(table, kobj) {
    return _.map(table, L.curry2(L.renameKeys)(kobj));
  };
  ...
  L.$.naturalJoin = function(table1, yrel) {
    var xks = _.keys(_.first(table1));
    var yks = _.keys(_.first(yrel));
    var ks  = _.intersection(xks, yks);

    var sz = (_.size(table1) <= _.size(yrel)) ? [table1, yrel] : [yrel, table1];
    var r  = _.first(sz);
    var s  = L.second(sz);
